...
class BHOT():
    ...
    def __init__(self, datasets_root = None):
        ...
        self.dataset_home = os.path.join(self.datasets_root, self.dataset_name, 'raw')
    ...
    def next_batch_sg(self, type = 'train'):
        ...
        target = np.ndarray(shape=[totalLength], dtype=np.int32)
        context = np.ndarray(shape=[totalLength], dtype=np.int32)
